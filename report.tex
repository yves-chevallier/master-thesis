...
We were inspired by Lang's group's method which would be to use a double adhesive tape to create a microchannel. The procedure is illustrated in figure \ref{slice}. Firstly, the glass cover is recovered with double adhesive tape. During this operation, close attention must be paid to ensure air is not trapped. The microchannel will have a width of about \SI{2}{\mm}. The sharp edge of the slice of glass can be used to cut off the excess tape. Then a glass cover is placed on the channel (5). Better adhesion is ensured by pressing firmly on both sides. The slice is then flipped upside down and with a dispenser, the colloidal solution can be applied in the opening of the microchannel. Through capillarity, the liquid will fill the channel (6). In the event of air bubbles, a small vacuum pipette can be placed at the other opening while the channel is being filled with the solution. Once the channel is filled properly, it can be sealed with glue by applying both edges with a small stick. With this method, the sample was able to last for a long time. 

\fig{slice}{Slice preparation}

The colloidal solution has to be diluted correctly. To proceed, concentrated spheres solution is added with water. \SI{200}{\micro\liter} is dropped on a cover slit and imaged by the microscope. The average amount of visible particles is counted and this value is written on the container. Depending on the concentration level, the solution can be adjusted to become more diluted. 
...
The experiment was successfully repeated with the \SI{1.5}{\um} spheres. However, the minimum trapping power was not measured. We also tried to trap the \SI{6}{\um} spheres but were not able to move them. They seem to be all stuck on the slit. 
...
Another laboratory from NUS was asked to grow Escherichia coli bacteria. These bacteria have an elongated rod shape and are suitable for rotation with a shaped optical trap. We use here an line of traps to get and rotate. Figure \ref{ecoli} shows some frames of a video recorded. 

\fig{ecoli}{Rotation of an e-coli bacterium with HOT} 
\fig{mitrotate}{Rotation of MIT logo with \SI{0.75}{\um} spheres}

The e-coli chosen here was particularly long. Our previous attempt with shortest ones was quite tough because the bacterium aim to align itself with the axial direction of the trapping beam. Figure \ref{ecoli2} shows on (a) several e-coli trapped in the vertical position in our 5 traps. In (b), we can observe the same case with two beams. 

\fig{ecoli2}{E-coli trapped in the vertical position}

With this experiment we succeed in the rotation of an organic object by the use of a shaped beam composed of an alignment of several traps. This lead to the same result we can obtain with the use of a cylindrical lens. 
