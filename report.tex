...
We can also notice on this figure that the imaging plane is matched to a $x'_0$ position on the observation plane. During the hologram recording, it is possible to add multiple gratings to match another depth on the imaging side to another $x'_0$ location on the observation plane. The figure \ref{vhlambda2} shows two planes (A and B) and their corresponding representation on the observation plane. Here the hologram has two gratings matching different depths. The field of view $\Delta L$ is still directly proportional to the illumination bandwidth $\Delta\lambda$. 
...
We note the two first objectives and the first relay are not necessary in our application. They are useful for structuring light applications. 
...
\section{Image consideration}
\todo{Add images of the debris and explain the shift}

\section{Illumination spectrum}
As seen before, the bandwith of the illumination is proportional to the field of view. Using a halogen white light source is far away and too broad. Practically, \SI{40}{\nm} is enough. Adding a filter 
\todo{continue here}
...
On the first attempt, we dropped \SI{100}{\micro\liter} of a diluted solution containing \SI{10}{\um} beads on a microscope slice. We covered it with a \SI{0.17}{\mm} glass cover slit. With this, we were unable to trap any spheres probably because the space between the slit and the cover slit was too small and the spheres were entrapped in between. 

For the second attempt, we used a silicon ring spacer of thickness \SI{0.8}{\mm} and a circular cover slit. \SI{300}{\micro\liter} was put on the slice. 
 
\todo{mesurer l'épaisseur du cover slit et du ring caoutchouc}
...
\subsection{Issues}
After many adjustments we were able to see a line under the microscope but the quality was not ideal. Moreover, the length of the line was larger than the field of view as shown on figure \ref{cylissue2} (a). In our setup the second lens of the second telescope is too far away from the objective lens. Without considerable changes to our setup, no solutions were found to reduce the size of the line trap. 
...
For each configuration, the image was measured at the intermediate image plane of the HOT setup by placing the camera there. The intensity of each spot was measured at its supposed pixel position with the sum of its 49 neighbor's pixels to prevent alignment issues. 

Figure \ref{targets} presents the five traps configuration used here. (a) Symmetric 8 traps pattern, (b) Random disposition of 8 traps, (c) Two perfectly symmetric traps, (d) A small array of 25 traps, (e) The point ring reference image from Holoeye. 

\fig{targets}{The five targets used in this experiment}

Based on Di-Leonardo's work \cite{DILEONARDO2007}, the efficiency, the uniformity and the standard deviation were measured for each configuration. Table \ref{eff1}, shows the average of the five targets. As we did not measure the total incident power on the camera, the efficiency is relative to the best results obtained for the SR algorithm with the 4th target. 

\begin{table}[h]
\begin{center}
    \begin{tabular}{|p{4cm}|c|c|c|} \hline        
        Algorithm & e    & u    & $\sigma$ \tabularnewline \hline
        RM        & 0.27\% & 0.33 & 66       \tabularnewline \hline
        S         & 0.61\% & 0.70 & 20       \tabularnewline \hline
        SR        & 0.73\% & 0.74 & 15       \tabularnewline \hline
        GSW       & 0.73\% & 0.68 & 16       \tabularnewline \hline
    \end{tabular}
    \caption{Average for all the targets \label{eff1}}
\end{center}
\end{table}

These qualitative measurements deserve to be repeated for more consistent results. Herein the SR algorithm shows better results than GSW. The possible causes could be an error in the algorithm or due to the way the intensity of each spot has been measured. In any case, it is clear that SR and GSW are better than S and RM. 

\subsection{Uniformity} 
The uniformity for each of the five targets is shown in the table \ref{unif}. 

\todo{Ces résultats sont vraiment curieux\ldots}
\begin{table}[h]
	\begin{center}
...
\hfill Yves Chevallier\\
\hfill \includegraphics{fig/sign_ycr.pdf}
...
This document was compiled with pdflatex and bibtex using Vim and vim-latex.
