...
SMART has previously published promising results of projected tomography of a transparent sample using \gls{tie}. More than that, \index{tie} TIE was successfully combined with a VHIS to retrieve phase information of an organic sample. Using these results to achieve tomography is a new step which is yet to be demonstrated.  
...
Prof. Christophe Moser from \gls{epfl} in Lausanne, \index{epfl}Switzerland contacted Prof. George Barbastathis of \gls{mit} 3D Optical Systems Group, in Boston, U.S., for my participation in this project to pursue my master's thesis. I was invited to work for 25 weeks in Singapore under the responsibility of SMART and CENSAM, and to collaborate in a team with Chen Zhi and Wensheng Chen and Prof. Yuan Luo. 
...
Optical tweezers have proven their capabilities for moving, stretching and rotating microscopic living objects. A significant number of research groups have designed different solutions to rotate such objects. For example, in 2008, M. K. Kreysing \emph{et al.} from the University of Leipzig, Germany, used a modified dual-beam laser trap delivered by two optical fibers to rotate a \index{red blood cell} red blood cell around an axis perpendicular to the optical direction \cite{KREYSING2008}. Nevertheless, this solution is challenging to implement and is not very versatile because the alignment of the two fibers are critical. Therefore, this project will focus on the rotation of micro-organisms with standard optical tweezers. 
...
The second part deals with the volume hologram setup. The alignment \index{alignment} of this system is very sensitive and every component has to be assembled carefully. In addition, compromises must be made to combine the volume hologram system with the tweezers setup due to their stringent requirements. 
...
This report is mainly divided into two parts. The first part is a theoretical approach of optical trapping, volume holography and \index{micro-fabrication} micro-fabrication based on scientific publications and books. Most of the illustrations found in this document are inspired from these readings but they were redrawn for better understanding and a better consistency. The second part is focused on experiments where results and issues are shown and explained. This part ends with a discussion of the results and suggests the possibility of further investigations. 
...
The detection of optical scattering and gradient forces on micrometer sized particles was first reported in 1970 by Arthur Ashkin \index{Ashkin}, a scientist who worked at Bell Laboratories. His work was used a decade later by Steven Chu in a laser cooling technique called optical molasses that can cool down neutral atoms by slowing them down with laser light. This research earned Steven Chu the Nobel Prize in Physics in 1997. 
...
Figure \ref{hotsetup} shows a schematic representation of a standard holographic optical tweezers (\gls{hot}) setup. Instead of using a galvano mirror, a diffractive optical element (\gls{doe}) is inserted at the Fourier plane of the light path in order to obtain an image of its gratings at the back focal plane of the objective. 
...
In addition, a \gls{lcos} display cannot be driven with a \gls{dc} voltage because it will cause a permanent rotation of the liquid crystal molecules. Most of the SLM does not control each pixel with an analog voltage due to the driving complexity. Typically, two references voltages can be chosen $V_{bright}$ and $V_{dark}$ which are two binary values that can be applied on each pixel. An addressing sequence modulates each pixel to obtain different phase levels. Because of bandwidth considerations, the refresh time is limited and a compromise has to be made between the number of phase levels and the refreshing rate.
...
\chapter{VHIS}
\section{Background}
Volume Holographic Imaging System was originally developed in early 2000 by Prof. George Barbastathis \cite{BARBA2002}. This imaging system exploits the Bragg selectivity by using a hologram where the thickness is much larger than the light wavelength. This is in contrast with confocal microscopy or optical coherent tomography which can also image subsurface tissue structure with minimally-invasive properties. A \gls{vhis} can see multi depth planes at the same time without scanning. The hologram can be seen as a depth-selective element. During the recording procedure of this element, by superimposing several gratings in the recording material, it is possible to choose how many planes can be imaged and at which depth they are located. 
...
Thus, the lateral field of view is proportional to the bandwidth of the illumination. Figure \ref{vhlambda} illustrates a basic \gls{vh} system composed of an imaging lens and a Fourier lens. Located at the Fourier plane of the screen, the thick hologram is placed at a certain angle. The two squares represent the matching location for both the imaging plane on the left and the observation plane on the right in terms of wavelength. In this example, the sample is illuminated with an uniform broad band white source (400-\SI{700}{\nm}). Therefore, on the observation plane, we can observe a rainbow pattern. Each wavelength component comes from a Bragg-matched x-position on the image plane. The wavelength selectivity is even better when the hologram is thick.

\figc{vhlambda}{VHIS, basic setup}{VHIS, basic setup illustration of field of view related to the illumination bandwidth}

We can also notice on this figure that the imaging plane is matched to a $x'_0$ position on the observation plane. During the hologram recording, it is possible to add multiple gratings to match another depth on the imaging side to another $x'_0$ location on the observation plane. The figure \ref{vhlambda2} shows two planes (A and B) and their corresponding representation on the observation plane. Here the hologram has two gratings matching different depths. The field of view $\Delta L$ is still directly proportional to the illumination bandwidth $\Delta\lambda$. The line color on this figure has nothing to do with the wavelength. 
...
We can also note the thick hologram is not exactly oriented at 45$^\circ$ in this figure. The reason is because the hologram is not a simple vertical thick grating but a slanted grating. In practice, the hologram is first oriented to an angle of about 45$^\circ$, then its orientation is finely adjusted. 

\section{Scale consideration}
On figure \ref{debris}, we can observe a dead cell focused on each individual plane. The planes are virtually separated with white lines. The plane separation is about \SI{50}{\um} and we can observe that the specimen is out of focus on the adjacent planes. That said, it can also be noticed that the specimen is shifted to the left on each plane. Therefore the translation stage needs to be moved if the specimen needs to be seen in its entirety. 
...
The aim of this experiment is to bring together the volume hologram setup with the holographic optical tweezers setup. Previously, we have succeed to build a VH setup with a two planes hologram and we demonstrated the functionality of our trapping system. This setup is intended to work with the micro fabricated tools which will come in the near future. 

To validate this setup, we need to be able to trap some spheres located at different planes and monitored with the VH system. Moreover, we should be capable to move a trapped sphere from one plane to another. Let us remember the separation between the two planes on the hologram is \SI{30}{\um} while we previously demonstrated a displacement of a trapped sphere over a distance of more than \SI{50}{\um}. 

Aware of the very low efficiency of the VHIS, we replaced the GE-1650 camera with an Andor iXon 888 which offer a fill factor of 100\% and a remarkably sensitivity. 

\subsection{Setup}
Figure \ref{setup3} shows the combination of the VH previously assembled and tested and the HOT tweezers part. We have performed some small changes on both setups in order to ensure the compatibility. Firstly, on the VH part, we changed the orientation of the swivel stage so that we can mount the Andor camera which is much bigger than the previous one. This adjustment is visible on figure \ref{hot-vh3d}. Secondly, we put an additional beam-splitter on the tweezers part in order to provide illumination for the VH through the 100x objective. 

During this experiment, we were compelled to replace the Coherent Verdi V10 with a less powerful model, a Coherent Verdi V6 because another research team needed it. The laser position is critial for all the setup alignment. Therefore, we needed to realign all the optical components after installed the new laser. 

Initially we wanted to use only one objective lens for both the VHIS and the HOT setup, however this change require recording a new hologram with the new setup and this operation require a specific laser. This is why we designed the setup in this way. 
...
\subsection{Alignment}
The alignment of the tweezers part was the easy part. All the components are clamped on the optical table and even the new laser required adjusting some of them, we easily managed to get good traps on the sample plane. However, the VH has to be adjust either in the z direction and the x and y direction. Because the whole VH is attached to the vertical damped pole, the x/y alignment is limited to the one degree of freedom. Moreover, the thick hologram can slightly be moved to adjust the viewing position. To face with the z-axis, we added a 30 mm cage fine translation stage (SM1Z) at the tweezers objective. Then we can precisely adjust the focus position. Also, we added another translation stage to move the whole VH vertically. 

\fig{targetaligned}{Alignement of the HOT+VH with a resolution target}

After long adjustments, we eventually succeed to image the 1951 USAF resolution target on both the VH and the HOT setup. Figure \ref{targetaligned} shows the image obtained on both sides. The target colors are reversed on the HOT side due to the transmission illumination. Concerning the second image, no band-pass filter was used to maximize the light intensity. The black circle is caused by the N.A. mismatch between the 50x objective and 100x. 

Once we aligned the setup, we tried to image the \SI{10}{\um} spheres but we could not see anything but white light. By comparing the size of the 7th group of the resolution target, we realized the spheres would be too small to get any images. The second objective lens from the VH was a Mitutoyo 10x that we replaced with a Mitutoyo 2x to increase the overall magnification. 

Despite this modification we still could not see anything. We then decided to trap and move a sphere to expect any variations on the VH. Figure \ref{firstspheresvh} shows in (a) and (b) two images from the VH with a sphere at a different position. The central image is the pixel difference between (a) and (b). Here, we can almost guess the shape of the sphere. 

\fig{firstspheresvh}{Sphere at two different positions seen with the VH}

Although the system is aligned, we got an important illumination issue that we tried to solve on the one hand by modifying the illumination part and on the other hand by using fluorescent spheres.

\subsection{Illumination issues}
The main problem is a N.A. mismatch between the two objectives used. Usually in microscopy, the condenser and the objective lens have both the same numerical aperture. In our case, the illumination comes from the 100x N.A. 1.4 and the sample is observed through a 50x N.A. 0.55. Figure \ref{illum} illustrate in (a) this problem. The illumination rays are shown in blue. Moreover, we can see that the cone of light that hit the top objective is very small and most of the illumination light is lost. This is one reason why we need the Andor camera. 
\fig{illum}{Illumination issue caused by N.A. mismatch}

Also, a proper illumination has to be focused at the back focal plane as shown on the second image (b). We slightly modified our setup to achieve this. Nevertheless, the illumination spot is in this case a collimated beam with a diameter limited by the objective front lens which will reduce the field of view on the VH. In addition we see that almost 75\% of the illumination light is lost at the two beam-splitters. 

In a further step, it worth replacing the top beam-splitter with a dichroic glass that reflect the trapping laser \SI{532}{\nm} and transmit the illumination which may be a LED source. The second beam-splitter can also be replaced with a dichroic filter that reflect the LED bandwidth and transmit another band from a second illumination. Because the spheres are almost transparent, a reflecting illumination will give bad results. In this experiment, we use a second white light source to shine the sample from the top to see the sphere with the GE-1650 camera. 

After modifying the tweezers illumination part, we realigned the system and acquired new images. Figure \ref{vhtw1} shows the group 7th with a better magnification but with a smaller field of view as the illumination is collimated. The two images (a) and (b) illustrate the focus on the target in both planes which have a separation of \SI{30}{\um}.

\fig{vhtw1}{Target imaged on the tweezers side}

In spite of these adjustments, we still could not see the spheres properly. We therefore decided to switch to fluorescence spheres. 

\subsection{Fluorescence}
We obtained \SI{15}{\um} fluorescent spheres that have an absorption peak at \SI{540}{\nm}. Their emission is located around \SI{560}{\nm}. We used the \SI{532}{\nm} trapping laser to pump the spheres and we added a notch filter that block the excitation wavelength before the Andor camera. Figure \ref{expbeads} shows on the left the spheres seen through the HOT objective lens in reflexion mode and on the right with fluorescence. Concerning the VH, this time we could observe the spheres as it is shown on figure \ref{fluo}. 

\figc{expbeads}{Spheres observed with fluorescences}{Spheres observed from the tweezers side in reflexion mode (a) and with fluorescence (b)}

A major drawback with these fluorospheres is their absorption at the trapping wavelength. Photobleaching occure rapidely and the lifetime of a trapped sphere is less than one minute. Moreover, fluorospheres are more difficult to trap. We obtained the best compromise between photobleaching and trapping efficiency by setting the laser to \SI{230}{\milli\watt} which correspond to approximatively \SI{2}{\milli\watt} per trap. 

\fig{fluo}{Fluorescent spheres seen from the VH}

\subsection{Setup Efficiency}
In the meanwhile, we measured the total trapping efficiency knowing that important optimisations can be made for instance with removing the first beamsplitter needed by another research group. The beam intensity was measured at different position and the results are given in table \ref{efficiencytable}. 
...
We notice only 8\% of the total power strike the imaging plane. This efficiency can be bring to 16\% without the first beamsplitter and by using dichroic mirrors we can expect to gain few more percent. The last column shows on the table states for the efficiency of the intermediate components. We observe for instance the SLM has an efficiency of 45\% and the components located after the spatial filter \emph{i.e.} mirrors, lenses and iris disperse and block 35\% of the incoming power. 

\clearpage
\subsection{Results}
As we still have illumination issues, we used fluorospheres for the rest of this experiment. Although these spheres are more difficult to trap due to their fluorescence coating, we managed to trap and move several spheres. Our first result is shown on figure \ref{final1} where we moved a sphere from one plane to another. On the first image a sphere can be observed in focus on the plane A and blurry on the other. On the last image of this video sequence, the sphere is on focus on the second plane, \SI{30}{\um} above the principal focal plane.\par 
...
\clearpage
\subsection{Conclusion}
Altough we demonstrated the trapping capabilities of micro spheres and the rotation of those ones along a arbitrary axis observed through the VHIS, three main drawbacks remains. 

First of all, no good illumination can be obtained with a condenser with a numerical aperture smaller than the imaging objective lens. In regular microscopes, the condenser is actually pretty big to collect as much possible light from the source. In our case, the back aperture of the 100x objective has a diameter of \SI{6}{\mm} which is very small. We also tried to illuminate the sample through the VH objective by replacing the right angle mirror with a beam-splitter. However, transparent objects such as biological cells and spheres do not scatter a lot of light and nothing can be seen on the VH camera. 

Secondly, we used a two-plane slanted hologram in this experiment. It turned out that this hologram has a very small efficiency (< 1\%). Because of this, only a very sensitive camera can be used to get some images. Even with quantum efficiency above 90\%, a fill factor of 100\% and a dark current below 0.001 electrons per pixel per second. We needed an exposure time of 0.9 second and an electron-Multiplying gain of 300 to see the fluorosphere. 

Lastly, it can be seen over the results we got, the VHIS gives a relatively poor image quality. The group 7 of the resolution target cannot be image optimally if compared with the images obtained from the tweezers part. 

However, the thick hologram has not been recorded on this setup which may lead to strong optical aberrations and poor efficiency. In a future experiment, one can imagine to record a new thick hologram with more and planes that offer a broader field of view. 

The illumination needs also to be reconsidered. A good option would be to use only one objective lens for both the VH and the HOT systems. In this case a proper Köhler illumination designed to match the imaging objective lens. Nevertheless, the objective requirements for both systems are in conflict. On one hand, the tweezers part needs a high numerical aperture and a small depth of field while on the other hand, the volume holographic imaging system require a long depth of field if the system is intended for a projection tomography. However, compromises may be found with slightly reducing the magnification factor and the N.A.

Concerning the tweezers part, we measured an overall trapping efficiency of 8\% because the laser output is split in half for two project and also because we did not pay much attention of the shaping of the beam \emph{i.e.} obtaining a high quality Gaussian beam that properly fit the SLM. 

To conclude, we succeed in combining a volume holographic imaging system (VHIS) and a holographic optical tweezers (HOT) for trapping and viewing particles at different depth simulaniously. 
...
\section{Hologram generation}
During the past five years, many researchers have tried to improve the hologram generation performances. They tried to minimize the overall phase change between frames which is more suitable for high stability requirement such as force measurement \cite{PERSSON2010}. They optimized their algorithms to generate holograms at faster frame rates and they designed new algorithms such as the weighted Gerchberg-Saxton. 

\subsection{Performances}
In the race for performance, developers realized they cannot use a \gls{pc} \gls{cpu} to compute hologram at high frame rates even if the implementation is done on a low level programming language (\gls{asm}, C). Nowadays, high performance Graphics Processing Units (\gls{gpu}) commonly used for video games or scientific computations feature a highly multi-core hardware. Their design is composed of hundreds of processor cores with a very fast access to a local and a global shared memory. 

The Open Graphics Library (OpenGL) or DirectX is a cross-platform \gls{api} for writing applications that produce 2D and 3D graphics by using the computational power of GPUs. These libraries are more dedicated for making applications such as \gls{cad} software or computers games. In its basic operation, OpenGL accepts primitives such as points, lines and polygons and converts them into pixels via a graphics pipeline. 

NVidia, a graphics accelerator manufacturer realized that the computational power of their products can also be used for applications other than 3D rendering. A Compute Unified Device Architecture (\gls{cuda}) was then developed to offer a more adapted architecture for non-graphical applications like cryptography or computational biology. Recent versions of Matlab and Mathematica support for instance CUDA. As an example, Martin Persson's work uses CUDA to generate holograms with the \gls{gsw} algorithm. Unfortunately, if CUDA seems to be an established standard for scientific computation, computational compatibility is highly dependent on the graphic card used and only Nvidia cards are compatible. 
...
With a background in electronic engineering, I'm surprised no autonomous hologram generation system exists. As we discussed earlier, pixels shader offer an interesting base to compute a trapping hologram because they work in parallel and save a lot of computation time. However, this approach is still very demanding in computational resources. A recent GPU needs to be used with a complex CUDA application to achieve hologram generation of an iterative algorithm. Even with this, the frame rate is limited to few tens of hertz. 

A Field-programmable gate array (FPGA) is a programmable integrated circuit that can be very suitable for the integration of this kind of algorithm. For instance, independent computational units can be created on the basis of a shader structure. However, in this case the structure will be more specialized than a Turing machine. The Fresnel forward and back propagation can be hard programmed in a non-sequential way which increases the performance by a lot. 

By using a dedicated platform to generate the SLM hologram it is possible to expect a high frame rate generation above \SI{200}{Hz} with smart optimisations. For example, we noticed in our benchmark of algorithms efficiency the algorithm that gives the better results depends strongly on the traps configuration and its symmetry. It can be interesting to choose the more adapted algorithm with regards to the trap configuration. 

Moreover, an embedded and dedicated platform may take the form of an external USB device with a DVI connector or, better, with a direct connection to a LCoS display.
\chapter{Discussion}
\section{Results Summary}
All the goals initially set in place for this project have been met although we did not succeed to rotate and image an organic specimen under the VHIS because of a illumination issue leading to a poor image quality. However, it can be inferred across the conducted experiments that pursuing our original goal remains perfectly feasible. 

Escherichia coli have been successfully rotated along an axis parallel with the trapping direction and micrometric spheres were rotated along an axis normal to the trapping direction. We demonstrated the trapping capabilities for sphere diameter in the range of \SI{0.7}{\um} and \SI{15}{\um}. 

Optimized holograms were produced with success with the help of Matlab and with the use of GPU for real-time trapping. In addition, SMART, has obtained the expertise with spatial light modulators concerning their use and their calibration. 

In addition, all the steps have been undertaken in order to make microfabricated SU-8 structures that would allow for holding and rotating a living cell. As we mentioned before, other research groups are also interested in microfabrication for optical tweezers applications. Microelectromechanical systems are a very promising area and it is worthily of study its technological possibilities. SMART is very fortunate to have at its disposal a NUS laboratory competent with two-photon photopolymerization lithography. 

\section{Further investigations}
If the decision is made to continue using a volume holographic imaging system to achieve projected tomography of a complex phase object, it is necessary to improve the overall efficiency and image quality of the VH part. This can be achieved with the recording of a new thick hologram. 

However, before to make this step, it may be worthwhile to try using only one objective lens for both trapping and imaging. In this way the illumination will be an easier part. 

We noted most biological materials absorb only weakly in the near infrared region of the electromagnetic spectrum, moving to a near-infrared laser to achieve optical trapping is conceivable now. Moreover, cheap solid-state lasers can be found at 808 or \SI{1064}{\nm}. It can be also interesting allows for dynamically adjusting the laser power with regards to the number of traps. The laser can be also shut off when no traps are required. In the case of using a infrared laser, safety measures have to be taken. The SLM needs to be enclosed to block all the high orders of diffraction. 

Demonstrating trapping capabilities with a microfabricated structure is a very exciting goal. When the new lithography setup from the Centre for Ion Beam Applications at NUS will be ready, a first microstructure will be created. The next actions concerning this part of the project were already mentioned in section \ref{2ppactions}. 

We did not focus on the tomography part during this project. Although TIE and VHIS have been used together to retrieve phase information of organic specimens and TIE projected tomography were demonstrated for a macroscopic object, we cannot ensure VHIS TIE Tomography is a doable solution. Therefore, I suggest spending more efforts on this particular aspect of the project. Limited depth of field and poor telecentricity might be some important issues. 

In addition, we observed during this project the importance of the Brownian movement for small objects in a colloidal solution. Good Random transform can be achieved only if the  projections taken at different angle are acquired around a fixed axis. The Brownian force will induce slow vibrations on an optically trapped particle. By monitoring the trapped particle with a camera, an high speed feedback control loop can limit the amplitude of these vibration. 

Finally, an important part of this project was devoted to build a microscopy setup. Mechanical stability issues encouraged us to use more adapted optomechanical parts. However, our setup is still pretty fragile in terms of stability and optical quality. Using a real microscope as a base for optical trapping should be the right solution, but for this such microscope must be first obtained. 

\clearpage
\section{Conclusion}
Overall, the achieved goals throughout this project has met the requirements that were given in the beginning, and has resulted in a fairly functional optical setup. As a result of the way this project was structured, the end result is mostly a proof of concept that needs further investigations.
 
If this project were to be developed further, several considerations mentioned above need to be overtaken. 

Considering the optical tweezers part our expertise is now sufficient to guarantee the final feasibility of the project which is a rotation of an organic specimen along an axis perpendicular to the imaging direction. One cannot be said for the VH system, the encountered issues are too important to be bypassed and a VHIS TIE projection tomography must be first demonstrated. 

As every research project, our investments could be endless. There is so many parameters to improve and an almost infinite amount of ideas to try. Thankfully, this master's thesis had a defined end. The results obtained even though they are not up to our expectations fulfilled or primary goals. This project is even more successful than SMART now has a fairly good knowledge with holographic optical tweezers. 
