function H = GSW(target, scale_target, size_hologram, pixel_hologram, lambda, f)  
    ...
    U2 = repmat(u.^2, hh,1);    V2 = repmat(v.^2, wh,1)';
    UV = (U2+V2)*pi/lambda/(f^2); % Quadratic gratigs (lens effect)
    x = s(:,1)*scale_target(2)/wt;  y = s(:,2)*scale_target(1)/ht;
    z = s(:,3);
    
    %% Local variables
    K = 10; 
    W = ones(M,1);      % Initialize weights
    P = rand(M,1)*2*pi; % Initialize compensation phase
    I = zeros(M,1);     % Initialize trap intensity
    Iold = I;     
    ...
    for k= 1:K 
        ...
        for m=1:M              
            D = x(m)*U + y(m)*V + z(m)*UV;  
            I(m) = mean2(exp(1i*(Q-D)));
        end

        % Adjust weights   
        W = W * mean(abs(I)) ./ abs(I); 
        P = angle(I);

        % Check if we can abord 
        if abs(sum(abs(Iold))-sum(abs(I))) < 1e-5, break, end         
        Iold = I;          
    end
end
