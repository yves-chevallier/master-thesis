function H = RM(target, scale_target, size_hologram, pixel_hologram, lambda, f)  
    %% Get spots position from  the target
    s = spots(target);     % Traps positions
    M = size(s,1);         % Number of traps    
    wt = size(target,2);   % Target Width in pixels
    ht = size(target,1);   % Target Height in pixels
    wh = size_hologram(2); % Hologram Width in pixels
    hh = size_hologram(1); % Hologram Height in pixels
    ...
    R = round(rand(hh,wh)*(M-1))+1;
    H = exp(1i*(x(R).*U + y(R).*V + z(R).*UV));    
return   
